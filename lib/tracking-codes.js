  ...
  const trackingCodes = () => new Promise((resolve, reject) => {
    ...
    return db.trackingCodes.getAll().catch(err => {
      ...
      const recursiveTracking = async (queue = 0) => {
        ...
        const serviceCode = String(code.service_code).startsWith('FR')
        ...
        if (ErrorMessage && (!Array.isArray(TrackingEvents) || !TrackingEvents.length)) {
          ...
          if (startWithError || ErrorMessage.startsWith('Objeto não encontrado na base de dados dos Correios')) {
            if (startWithError || diffInDays(code.created_at) >= 15) {
        ...
        if (trackingEvent && parseInt(code.tracking_status, 10) !== trackingEvent.status) {
          try {
            ...
            if (err.response) {
              ...
              logger.error('@ERROR:', JSON.stringify(payload, null, 2))
            } else {
